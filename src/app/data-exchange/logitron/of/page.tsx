...
export default function LogitronOrdreFabricationPage() {
  ...
  useEffect(() => {
    const loadQuery = async () => {
      try {
        const q = await invoke<string>("get_sql_query", { formatName: "LOGITRON_ORDRE_FABRICATION" });
        setSqlQuery(q);
      } catch (e) {
    ...
    // load saved path
    const savedPath = localStorage.getItem("logitron_of_output_path");
    if (savedPath) {
      setOutputPath(savedPath);
    }

    const savedAuto = localStorage.getItem("logitron_of_auto_enabled");
    if (savedAuto !== null) {
      setAutoEnabled(savedAuto === "true");
    }
    const savedInterval = localStorage.getItem("logitron_of_auto_interval_sec");
    if (savedInterval) {
  ...
  useEffect(() => {
    localStorage.setItem("logitron_of_output_path", outputPath);
  }, [outputPath]);

  useEffect(() => {
    localStorage.setItem("logitron_of_auto_enabled", autoEnabled.toString());
  }, [autoEnabled]);

  useEffect(() => {
    if (Number.isFinite(autoIntervalSec) && autoIntervalSec > 0) {
      localStorage.setItem("logitron_of_auto_interval_sec", autoIntervalSec.toString());
    }
  }, [autoIntervalSec]);
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await invoke<ExportDatResult>("export_ordre_fabrication_dat", {
  ...
  const handleSaveQuery = async () => {
    ...
    setSaveStatus(null);
    try {
      await invoke("save_sql_query", { formatName: "LOGITRON_ORDRE_FABRICATION", queryTemplate: sqlQuery });
  ...
  const handleResetQuery = async () => {
    ...
    setSaveStatus(null);
    try {
      await invoke("reset_sql_query", { formatName: "LOGITRON_ORDRE_FABRICATION" });
      const q = await invoke<string>("get_sql_query", { formatName: "LOGITRON_ORDRE_FABRICATION" });
      setSqlQuery(q);
    ...
    <div className="p-8 flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "var(--text-primary)" }}>
          Data exchange · Logitron · Ordre de Fabrication
        </h1>
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          Génère le fichier semicolon-separated <code>ordre_fabrication.dat</code> depuis SQL Server.
        </p>
      </div>
        ...
        <Card className="bg-(--bg-secondary) border border-(--border-default)">
          <CardHeader className="flex justify-between items-start gap-4">
            <div className="flex flex-col gap-1">
              <h2 className="font-semibold" style={{ color: "var(--text-primary)" }}>
                Requête SQL (Logitron · Ordre de Fabrication)
              </h2>
              <p className="text-xs" style={{ color: "var(--text-tertiary)" }}>
