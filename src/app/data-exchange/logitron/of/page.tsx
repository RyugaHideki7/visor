...
export default function LogitronOrdreFabricationPage() {
  ...
  const taskState = tasks["LOGITRON_OF"];
  ...
  useEffect(() => {
    ...
    async function setupListener() {
        unlistenResult = await listen<ExportDatResult>("logitron-of-export-result", (event) => {
        ...
        unlistenError = await listen<string>("logitron-of-export-error", (event) => {
  ...
  const handleOutputPathChange = (val: string) => {
    setTaskParam("LOGITRON_OF", val);
  };

  const handleAutoToggle = async (enabled: boolean) => {
    if (!taskState) return;
    if (enabled) {
        await startTask("LOGITRON_OF", taskState.intervalMinutes, outputPath);
    } else {
        await stopTask("LOGITRON_OF");
    }
  };
  ...
  const handleIntervalChange = async (v: number) => {
    ...
    setTaskInterval("LOGITRON_OF", newInterval);
    if (taskState.running) {
        await stopTask("LOGITRON_OF");
        await startTask("LOGITRON_OF", newInterval, outputPath);
    }
  };
